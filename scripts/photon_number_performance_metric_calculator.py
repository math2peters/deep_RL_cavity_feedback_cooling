#!/usr/bin/env python3
"""
PHOTON NUMBER Performance Metric Calculator

Script to calculate performance metrics for PHOTON NUMBER sweep data
Analyzes peak performance, range at 50% of peak, and relative performance
across survival probability, cooling timescale, and 20-step energy metrics

This script is specifically designed for photon number parameter sweeps.
For detuning analysis, use detuning_performance_metric_calculator.py
"""
...
def find_peak_performance(photon_values, performance_values, metric_type):
    ...
    valid_photon = photon_values[valid_mask]
    ...
    return valid_photon[peak_idx], valid_performance[peak_idx], peak_idx


def find_performance_range(photon_values, performance_values, peak_photon, peak_performance, metric_type, performance_threshold=0.5):
    """
    Find the photon number range where performance reaches specified threshold of peak value
    
    Args:
        photon_values: Array of photon number values
        performance_values: Array of performance values
        peak_photon: Photon Counts at peak performance
    ...
    Returns:
        Tuple of (left_photon_number, right_photon_number, range_width)
    """
    # Remove NaN values and sort by photon number
    valid_mask = ~np.isnan(performance_values)
    ...
    # Sort by photon number
    sort_idx = np.argsort(valid_photon)
    sorted_photon = valid_photon[sort_idx]
    ...
    try:
        ...
        interp_func = interp1d(sorted_photon, sorted_performance, 
                              kind='linear', bounds_error=False, fill_value='extrapolate')
        
        # Check performance at boundaries
        min_photon = sorted_photon.min()
        max_photon = sorted_photon.max()
        performance_at_min = interp_func(min_photon)
        performance_at_max = interp_func(max_photon)
        ...
        left_crossing = min_photon if min_above_threshold else np.nan
        right_crossing = max_photon if max_above_threshold else np.nan
        
        # Find crossings with target performance
        # We'll sample the interpolated function and find crossings
        photon_fine = np.linspace(min_photon, max_photon, 1000)
        performance_fine = interp_func(photon_fine)
        ...
        if len(crossings) > 0:
            ...
            crossing_photons = photon_fine[crossings]
            
            # Find leftmost and rightmost crossings relative to peak
            left_crossings = [p for p in crossing_photons if p < peak_photon]
            right_crossings = [p for p in crossing_photons if p > peak_photon]
...
def calculate_performance_metric(results, param_name='photon_number', performance_threshold=0.5):
    ...
    Args:
        ...
        param_name: Parameter name ('photon_number')
    ...
    if param_name != 'photon_number':
        print("Warning: Performance metric is designed for photon number sweeps only")
        return {}
    ...
    model_metrics = {}
    global_ranges = {}  # Store global min/max photon number for each metric
    ...
    for metric_name, metric_type, metric_label in metrics:
        ...
        for model_folder, df in results.items():
            ...
            photon_vals = valid_df[param_name].values
            performance_vals = valid_df[metric_name].values
            
            all_photons.extend(photon_vals)
            
            # Find peak for this model
            peak_photon, peak_perf, _ = find_peak_performance(photon_vals, performance_vals, metric_type)
            if not np.isnan(peak_perf):
                all_peaks.append(peak_perf)
                
                # Find range width for this model
                left_photon, right_photon, range_width = find_performance_range(
                    photon_vals, performance_vals, peak_photon, peak_perf, metric_type, performance_threshold
                )
                if not np.isnan(range_width):
                    all_widths.append(range_width)
        
        if all_photons and all_peaks:
            global_ranges[metric_name] = (min(all_photons), max(all_photons))
            if metric_type == 'maximize':
        ...
        print(f"{metric_label}: Global photon number range = {global_ranges.get(metric_name, (np.nan, np.nan))}")
    ...
    for model_folder, df in results.items():
        ...
        for metric_name, metric_type, metric_label in metrics:
            ...
            peak_photon, peak_perf, peak_idx = find_peak_performance(photon_vals, performance_vals, metric_type)
            ...
            if np.isnan(range_width):
                ...
                print(f"  {metric_label}: Could not determine {threshold_percent}% range")
                model_metrics[model_folder]['metric_scores'][metric_name] = np.nan
                continue
            ...
            model_metrics[model_folder]['details'][metric_name] = {
                'peak_photon_number': peak_photon,
                'peak_performance': peak_perf,
                'left_50_photon_number': left_photon,
                'right_50_photon_number': right_photon,
            ...
            print(f"    Peak: {peak_perf:.3f} at {peak_photon:.1f} photons")
            print(f"    {threshold_percent}% range: {left_photon:.1f} to {right_photon:.1f} photons (width: {range_width:.1f})")
...
def main():
    """Main function to calculate and report PHOTON NUMBER performance metrics"""
    import argparse
    
    parser = argparse.ArgumentParser(description='Calculate PHOTON NUMBER performance metrics')
    ...
    # Load results for all models (photon number sweeps only)
    model_folders = ['mlp_sim', 'differentiator', 'mlp_experimental']
    
    print("="*60)
    print("PHOTON NUMBER SWEEP PERFORMANCE METRIC ANALYSIS")
    print(f"Performance Threshold: {threshold_percent}% of peak")
    print("="*60)
    print("Loading photon number sweep data...")
    results = load_sweep_results(script_dir, model_folders, param_name='photon_number')
    
    # Load 20-step energy data from energy traces
    print("Loading 20-step energy data...")
    energy_results = load_20_step_energy_from_traces(script_dir, model_folders, param_name='photon_number')
    ...
    cooling_results = fit_cooling_timescale_from_trace(script_dir, model_folders, param_name='photon_number')
    
    # Merge fitted cooling data with main results
    results = merge_cooling_data_with_sweep_results(results, cooling_results)
    
    if not results:
        print("No data found for photon number sweeps")
        return
    
    # Calculate performance metrics
    model_metrics = calculate_performance_metric(results, param_name='photon_number', performance_threshold=performance_threshold)
    
    # Print summary
    print_performance_summary(model_metrics, performance_threshold)
    
    # Save detailed results to CSV
    output_file = script_dir / 'photon_number_performance_metrics.csv'
    save_detailed_results(model_metrics, output_file)
    print(f"\nDetailed PHOTON NUMBER results saved to: {output_file}")
